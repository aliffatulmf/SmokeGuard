...
def main():
    ...
    parser.add_argument(
        "-c", "--clear-cache", action="store_true", help="Clean the Python cache folder"
    )
    parser.add_argument(
        "-i", "--install-deps", action="store_true", help="Install YOLOv5 dependencies"
    )
    fmt_group = parser.add_argument_group(title="format arguments")
    fmt_group.add_argument(
        "-f", "--fmt-input", type=str, help="Input file or directory to format"
    )
